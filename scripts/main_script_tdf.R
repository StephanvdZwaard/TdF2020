# -------------------------------- Data viz 1.0 TdF ------------------------------------------#
#                                                                                             #
# Omschrijving: Script for data visualization of TdF stages and power                         #
...
# --------------------------------- Data collection -----------------------------------------
  ...
  tdf_data <- read_excel("data/tdf-po-data.xlsx")
  
  
  # Scrape stage characteristics from Wikipedia
  webpage   <- read_html("https://en.wikipedia.org/wiki/2020_Tour_de_France")
...
# ------------------------------- Data preprocessing ----------------------------------------
  ...
  tdf_data <- tdf_data %>% pivot_longer(`Stage 1`: `Stage 21`,
                                         names_to  = "Stage",
                                         values_to = "Power") %>%
...
# ------------------------------- Data visualization ----------------------------------------
    ...
    # Create image with ggplot
     p <- ggplot(tdf_data, aes(x=Power, y=reorder(Stage,-as.numeric(Stage)), fill = stage_description)) +
     ...
     # Export image
     png(file=paste0("results/TdF2020.png"), width=160,height=160,units = "mm", bg = "transparent", res = 200, type = "quartz")
     p
     dev.off()
